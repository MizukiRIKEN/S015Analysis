#include "Assemble_flwv3.h"
//----------------------------------------------------------------------
// Macro: Assemble_flwv3.C
...
void Assemble_flwv3(Int_t nevt = -1)
{
  //////////////////////////////////////////////////////////
  ...
  for (Int_t i=0; i<nEntry;i++) {
    ...
    // --- BDC offset --------------------
    SetBeamOnTarget();
    //    if(BeamonTarget->X() == -999. || BeamonTarget->Y() == -999.) continue;
	...
	bevt[1] = 1;
      ...
      //else {
	//	cout << " negative id " << parentvid << " " 
	//	     << vertex->GetPos().X() << " : " << vertex->GetPos().Y() << endl;
	//	continue;
      //}

      new(ptpcParticle[mtrack]) STParticle();      
      STParticle *aParticle = (STParticle*)tpcParticle->At(mtrack);
	...
	aParticle->RotateAlongBeamDirection(ProjA/1000., ProjB/1000.);
      ...
      if(CheckBDCvsTrackCorrelation(vertex->GetPos())) 
	ntrack[1]++; 
     else
	aParticle->SetBDCCorrelationFlag(0);
	...
	ntrack[2]++;
      ...
      if(!bevt[0]) aParticle->SetBeamonTargetFlag(0);

      if(CheckVertexZ(vertex->GetPos().Z()))                                                bevt[2] = 1;
      else aParticle->SetVertexZAtTargetFlag(0);

      if(CheckVertexXY(TVector2(vertex->GetPos().X(), vertex->GetPos().Y())))  	            bevt[3] = 1;
      else aParticle->SetVertexAtTargetFlag(0);
      
      if(CheckBDCvsVertexCorrelation(TVector2(vertex->GetPos().X(), vertex->GetPos().Y()))) bevt[4] = 1;
      else aParticle->SetVertexBDCCorrelationFlag(0);

      aParticle->SetdEdxPointSizeCut(15);
      if(aParticle->GetdEdxPointSizeFlag()) ntrack[4]++; 
      ...
      // 	   << " best flag "                   <<  aParticle->GetBestTrackFlag()    
      // 	   << endl;
    ...
    //  cout << " %2 " << mtrack << " / " << ntrack[0] << " -> " << tpcParticle->GetEntries()
    //	 << endl;

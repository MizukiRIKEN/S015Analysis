...
void AsmFlw_getEvents(Long64_t nmax = -1)
{
  ...
  for(Long64_t ievt = 0; ievt < maxProc; ievt++){
	...
	if(aPart1!=NULL && aPart1->GetBestTrackFlag() && aPart1->GetLinearPID()>1000) {
	  ...
	  STParticle *nPart1 = (STParticle*) nParticleArray->At(numGoodTrack);

	  ResetPID(nPart1);

	  numGoodTrack++;
	  
	  nPart1->SetReactionPlaneFlag(1);

	  TVector3 mom3  = nPart1->GetMomentum();
	  TVector2 momPt = TVector2( nPart1->GetMomentum().X(), nPart1->GetMomentum().Y() );

	  // Rotate along beam angle
	  if( BeamAngle )
	    RotateAsBeamAngle(nPart1, &mom3, &momPt);

	  if( bMix ) {
	    nPart1->SetMixedEventID(mixEvt);
